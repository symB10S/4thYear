...
detector =  vision.ForegroundDetector(...
            ...
            'NumGaussians',2);
        %frames - gaussians
        %     3 - 3  = good for end = fast training rate
        %     3 - 7  = also good, small detections 
...
%     frame_subtracted = imbinarize(frame_subtracted,'adaptive','Sensitivity',0.2);
    ...
    % MASK IMAGE
%     frame_subtracted = rgb2gray( frame_resized ); % for (3) as creation of this variable
%     frame_subtracted(~mask) = 0;
    frame_resized = bsxfun(@times, frame_resized, cast(mask, 'like', frame_resized));

    % (3)----> MATLAB Forground Detector
    frame_subtracted = detector(frame_resized);
